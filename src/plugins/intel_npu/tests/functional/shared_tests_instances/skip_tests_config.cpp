...
std::vector<std::string> disabledTestPatterns() {
    ...
    static const auto skipRegistry = []() {
        ...
        _skipRegistry.addPatterns(backendName.isZero() && devices.has5010(),
                "WinDrv 32.0.201.5720/5737 Failures", {
                ".*OVInferRequestPerfCountersTest.*",
        });

        // [Tracking number : E#145347]
        _skipRegistry.addPatterns(backendName.isZero() && (devices.has4000() || devices.has5010()),
        ...
        _skipRegistry.addPatterns(devices.has6010(),
         ...
         // [Tracking number E#145573]
         _skipRegistry.addPatterns(devices.has6010(),
                "Failed to run InferenceManagerDemo (-2) : Terminated", {
                ".*smoke_precommit_LSTMSequencePt/LSTMSequenceLayerTestCommon.NPU6010.*",
                ".*smoke_precommit_LSTMSequenceCommonZeroClip/LSTMSequenceLayerTestCommon.NPU6010.*",
        });

        // [Tracking number E#150330]
         _skipRegistry.addPatterns(devices.has6010(),
                "Unexpected error message, quitting.", {
                ".*AvgPoolWithTanhTest.NPU6010.*",
        });
	...
	_skipRegistry.addPatterns("Skip tests that can not wrong when DRIVER is default compiler type", {
        ...
        // [Track number: E#151535]
        _skipRegistry.addPatterns(devices.has5010(),
                "code 0x78000004 - generic error code for invalid arguments", {
                ".*MaxPoolWithF16ToF32ConvertTest.NPU5010_HW.*"
        });
